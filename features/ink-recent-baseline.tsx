...
const RecentBaselineApp: React.FC<{ onExit: (result: RecentBaselineResult) => void }> = ({ onExit }) => {
  const [recentHighFeatures, setRecentHighFeatures] = useState<FeatureWithKey[]>([]);
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (totalCount > 0 && selectedIndex < totalCount) {
        onExit({ selectedFeature: allFeatures[selectedIndex], cancelled: false });
      }
      return;
    }
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box marginBottom={1}>
        <Text>
          {t('recentBaselineNavigationInstructions')}
        </Text>
      </Box>
      ...
      {totalCount > VISIBLE_ITEMS && (
        <Box marginTop={1}>
          <Text color="gray">
            {t('baselineScrollInstructions', visibleStart + 1, Math.min(visibleStart + VISIBLE_ITEMS, totalCount))}
          </Text>
        </Box>
      )}
    </Box>
...
export function startInkRecentBaseline(): Promise<RecentBaselineResult> {
  return new Promise((resolve) => {
    ...
    const { unmount } = render(<RecentBaselineApp onExit={handleExit} />, options);
