...
const InkSearchPaginationApp: React.FC<{
	onExit: (result: SearchPaginationResult) => void;
}> = ({ onExit }) => {
	...
	// 全機能リスト
	const allFeatures: FeatureWithKey[] = Object.entries(features).map(
		([key, feature]) => ({
			key,
			...feature,
		}),
	...
	useInput((input, key) => {
		...
		if (key.return) {
			...
			if (
				...
				selectedIndex < currentPageFeatures.length
			) {
				onExit({
					selectedFeature: currentPageFeatures[selectedIndex],
					cancelled: false,
				});
			}
			return;
		...
		if (key.downArrow) {
			...
			setSelectedIndex((prev) =>
				Math.min(currentPageFeatures.length - 1, prev + 1),
			);
			return;
		}
	...
	return (
		<Box flexDirection="column">
			...
			{currentPageFeatures.length === 0 && searchTerm && (
				<Box marginTop={1}>
					<Text color="gray">{t("searchTryOtherKeywords")}</Text>
				</Box>
			)}
		</Box>
	);
...
export function startInkSearchPagination(): Promise<SearchPaginationResult> {
	return new Promise((resolve) => {
		...
		const { unmount } = render(
			<InkSearchPaginationApp onExit={handleExit} />,
			options,
		);
