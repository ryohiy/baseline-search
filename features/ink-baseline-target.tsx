...
// 年選択コンポーネント
const YearSelectionApp: React.FC<{ onExit: (result: YearSelectionResult) => void }> = ({ onExit }) => {
  ...
  useEffect(() => {
    ...
    Object.entries(features).forEach(([key, feature]) => {
      if (feature.kind === 'feature' && feature.status && feature.status.baseline_low_date) {
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (availableYears.length > 0 && selectedIndex < availableYears.length) {
        onExit({ selectedYear: availableYears[selectedIndex], cancelled: false });
      }
      return;
    }
    ...
    <Box flexDirection="column">
      ...
      {availableYears.length > 0 && (
        <Box flexDirection="column">
          ...
          {availableYears.slice(visibleStart, visibleStart + VISIBLE_ITEMS).map((year, index) => {
...
const FeatureSelectionApp: React.FC<{ 
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (featuresForYear.length > 0 && selectedIndex < featuresForYear.length) {
        onExit({ selectedFeature: featuresForYear[selectedIndex], cancelled: false });
      }
      return;
    }
    ...
    if (key.downArrow) {
      setSelectedIndex(prev => Math.min(featuresForYear.length - 1, prev + 1));
      return;
    }
  });
      ...
      {featuresForYear.length > 0 && (
        <Box flexDirection="column">
          ...
          {featuresForYear.length > VISIBLE_ITEMS && (
            <Box marginTop={1}>
              <Text color="gray">
                ↑↓キーでスクロール (表示中: {visibleStart + 1}-{Math.min(visibleStart + VISIBLE_ITEMS, featuresForYear.length)})
              </Text>
            </Box>
          )}
        </Box>
...
export function startYearSelection(): Promise<YearSelectionResult> {
  return new Promise((resolve) => {
    ...
    const { unmount } = render(<YearSelectionApp onExit={handleExit} />, options);
    unmountFunction = unmount;
  });
}

// 機能選択開始関数
export function startFeatureSelection(year: string): Promise<FeatureSelectionResult> {
  return new Promise((resolve) => {
    let unmountFunction: (() => void) | null = null;
