...
const InkFeatureDetailApp: React.FC<FeatureDetailProps> = ({
	featureKey,
	featureData,
	onExit,
}) => {
	...
	const renderFeatureContent = () => {
		...
		if (featureData.kind === "feature") {
						...
						<Box marginLeft={2} flexDirection="column">
							...
							{featureData.status.support && (
								<Box flexDirection="column">
									...
									{Object.entries(featureData.status.support).map(
										([browser, version]) => (
							...
							{featureData.status.by_compat_key && (
								<Box flexDirection="column">
									...
									{Object.entries(featureData.status.by_compat_key).map(
												...
												<Box marginLeft={2} flexDirection="column">
													...
													{status.support && (
														<Box flexDirection="column">
															...
															{Object.entries(status.support).map(
																([browser, version]) => (
																	<Box key={browser} marginLeft={2}>
																		<Text color="gray">{browser}: </Text>
																		<Text>{version as string}</Text>
			...
			if (
				featureData.compat_features &&
				featureData.compat_features.length > 0
			) {
				content.push(
					<Box key="compat_features" marginBottom={1} flexDirection="column">
						<Text color="cyan">compat_features:</Text>
						{featureData.compat_features.map(
							(compatFeature: string, index: number) => {
				...
				content.push(
					<Box key="discouraged" marginBottom={1} flexDirection="column">
						...
						<Box marginLeft={2} flexDirection="column">
							...
							{featureData.discouraged.alternatives &&
								featureData.discouraged.alternatives.length > 0 && (
									<>
			...
			if (
				featureData.redirect_targets &&
				featureData.redirect_targets.length > 0
			) {
				content.push(
...
export function startInkFeatureDetail(
	featureKey: string,
	featureData: any,
): Promise<void> {
